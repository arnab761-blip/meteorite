    ...
    if (isLogin) {
      ...
      try {
        const res = await fetch('/api/signup', {
          ...
          body: JSON.stringify({ name, email, password }), // ইমেইল পাঠানো হচ্ছে
        });
        ...
        if (res.ok) {
          ...
          showToast("Email or Name already taken!", "error");
        }
      } catch (error) {
  ...
  return (
    <main className="flex flex-col items-center justify-center pt-24 px-4 min-h-screen relative">
      ...
      <div className="backdrop-blur-xl bg-black/40 p-8 md:p-12 rounded-3xl border border-white/10 w-full max-w-md shadow-[0_0_50px_rgba(168,85,247,0.15)] relative overflow-hidden mt-10">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 relative z-10">
          {!isLogin && (
            <div>
              ...
              <input required type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-500 transition-colors" placeholder="e.g. Arnab" />
            </div>
          )}
          
          <div>
            <label className="text-sm text-gray-300 mb-1 block">Email Address</label>
            <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-500 transition-colors" placeholder="explorer@universe.com" />
          </div>
          
          <div>
            <label className="text-sm text-gray-300 mb-1 block">Password</label>
            <input required type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-purple-500 transition-colors" placeholder="••••••••" />
          </div>

          <button type="submit" disabled={isLoading} className="w-full bg-gradient-to-r from-cyan-500 to-purple-500 hover:from-cyan-600 hover:to-purple-600 py-3.5 rounded-xl font-bold text-white mt-2 transition-all shadow-lg disabled:opacity-50">
        ...
        <button type="button" onClick={() => signIn("google", { callbackUrl: "/" })} className="w-full bg-white/5 hover:bg-white/10 border border-white/10 py-3 rounded-xl flex items-center justify-center gap-3 transition-colors relative z-10 text-sm font-medium">
        ...
        <p className="text-center text-sm text-gray-400 mt-6 relative z-10">
          ...
          <button type="button" onClick={() => setIsLogin(!isLogin)} className="text-cyan-400 hover:text-cyan-300 font-semibold">
